...
def kategoriOku(goster=False):
    # kategori dosyasını okuyalım.
    kategori = {}
    with open('kategori.txt') as f:
        for line in f:
            satir = line.strip().split(':')
            k = satir[0].strip()
            v = satir[1].strip().split(',')
            kategori[k] = v
...
def sozlukKategoriEsle(sozluk, kategoriler, goster=False):
    esle = []
    for s in sozluk:
    ...
    if goster:
        print('Sozluk Kategorileri:', esle)
...
def kategoriBul(SozKat, w, goster=True):
    sonuc = {}
    boy = len(SozKat)
    for i in w:
        # Satırdaki en büyük değerin yeri bulunacak.
        maxdeger = -1
        maxyer = -1
        liste = w[i]
        for yer in range(boy):
            if liste[yer] >= maxdeger:
                maxdeger = liste[yer]
                maxyer = yer

        sonuc[i] = SozKat[maxyer]
